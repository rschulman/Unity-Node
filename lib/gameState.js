(function() {
  var Level, gameState;

  Level = require('./level');

  gameState = (function() {
    var levels, players;

    players = {};

    levels = [];

    function gameState(levelobject, dlvl) {
      levels[dlvl] = levelobject;
    }

    gameState.prototype.addPlayer = function(id, player) {
      return players[id] = player;
    };

    gameState.prototype.getPlayer = function(id) {
      return players[id];
    };

    gameState.prototype.getLevel = function(dlvl) {
      if (levels[dlvl] != null) {
        return levels[dlvl];
      } else {
        return false;
      }
    };

    gameState.prototype.addLevel = function(dlvl, level) {
      return levels[dlvl] = level;
    };

    gameState.prototype.playerLogOut = function(id) {
      return delete players[id];
    };

    gameState.prototype.playerLevelMove = function(id, direction, levelCollection, callback) {
      var ascending, mover, newwhere, where;
      mover = players[id];
      where = mover.getLevel();
      console.log(mover.name + " moving from " + where + ", " + direction);
      newwhere = 0;
      ascending = false;
      if (levels[where].canLeave(id, direction)) {
        if (direction === "up") {
          newwhere = where - 1;
          ascending = true;
        } else {
          newwhere = where + 1;
        }
        if (levels[newwhere] != null) {
          console.log("New level in memory.");
          levels[newwhere].addPlayer(id, mover, {
            comingup: ascending,
            stairs: true
          });
          mover.setDlvl(newwhere);
          callback(where, newwhere);
        } else {
          console.log("New level NOT in memory.");
          levelCollection.find({
            dlvl: newwhere
          }, function(err, cursor) {
            cursor.count(function(err, number) {
              if (number === 1) {
                console.log("Level exists in DB");
                cursor.nextObject(function(err, loadlevel) {
                  levels[newwhere] = new Level({
                    generate: false
                  }, loadlevel);
                  return console.log("Loaded level " + newwhere + " from db");
                });
              } else {
                console.log("Generating new level.");
                levels[newwhere] = new Level({
                  generate: true,
                  type: "dungeon"
                }, []);
                levels[newwhere].save(levelCollection, newwhere);
              }
              true;
              levels[newwhere].addPlayer(id, mover, {
                comingup: ascending,
                stairs: true
              });
              mover.setDlvl(newwhere);
              return callback(where, newwhere);
            });
            return true;
          });
        }
      }
      return true;
    };

    return gameState;

  })();

  module.exports = gameState;

}).call(this);
