(function() {
  ...
  Level = (function() {
    ...
    Level.prototype.povObject = function(id) {
      var centerx, centery, checking, directionx, directiony, dist, elements, item, nextinrayx, nextinrayy, presensecheck, radian, subject, throwaway, vision, wallbug, xmove, ymove;
      vision = 8;
      radian = 0;
      subject = players[id];
      elements = {
      ...
      if (players[id]) {
        while (radian <= 2 * Math.PI) {
          ...
          for (dist = 1; 1 <= vision ? dist <= vision : dist >= vision; 1 <= vision ? dist++ : dist--) {
            ...
            for (throwaway in players) {
              checking = players[throwaway];
